        ...
        for (const prog of progKeys) {
            ...
            for (const wk of weekKeys) {
                ...
                if (todosArchivados) {
                    ...
                    // Ordenar por cantidad de días asistidos (de mayor a menor)
    ...
    try {
        ...
        let htmlFechas = `
            <div class="d-flex justify-content-end mb-3">
                <button class="btn btn-outline-danger btn-sm fw-bold" onclick="window.liberarFechasSorteo()">🔓 Restaurar fechas bloqueadas (Para grabar de nuevo)</button>
            </div>
        `;
...
document.getElementById('btnRealizarSorteo').addEventListener('click', async () => {
    ...
    try {
        ...
        setTimeout(() => {
            ...
            let contGanador = document.getElementById('ganadorFechas').parentElement;
            if (!document.getElementById('btnConfirmarSorteo')) {
                let btnConf = document.createElement('button');
                btnConf.id = 'btnConfirmarSorteo';
                btnConf.className = 'btn btn-success fw-bold w-100 mt-4 fs-5 shadow';
                btnConf.innerHTML = '✅ Validar Ganador y Bloquear Fechas';
                contGanador.appendChild(btnConf);
            }
            
            let btnConf = document.getElementById('btnConfirmarSorteo');
            btnConf.classList.remove('d-none');
            btnConf.onclick = async () => {
                if(confirm("¿Confirmas a este ganador definitivo? Las fechas se bloquearán para futuros sorteos.")) {
                    btnConf.innerText = "Bloqueando...";
                    let updatesSorteo = {};
                    fechasSeleccionadas.forEach(f => updatesSorteo[`6_sorteos_fechas_usadas/${f}`] = true);
                    await update(ref(db), updatesSorteo);
                    alert("¡Fechas bloqueadas exitosamente! Sorteo cerrado.");
                    btnConf.classList.add('d-none');
                    document.getElementById('resultadoSorteo').classList.add('d-none');
                    document.getElementById('sorteo-tab').click();
                }
            };

            document.getElementById('resultadoSorteo').classList.remove('d-none');
            btnSorteo.innerText = "🔄 Resortear (Aún no se bloquean)";
            btnSorteo.disabled = false;
        }, 2000);
...
window.liberarFechasSorteo = async function() {
    if(confirm("⚠️ ¿Estás seguro de liberar TODAS las fechas bloqueadas?\nVolverán a aparecer en la lista para que puedas grabar el sorteo de nuevo.")) {
        try {
            await remove(ref(db, '6_sorteos_fechas_usadas'));
            alert("✅ Fechas restauradas con éxito.");
            document.getElementById('sorteo-tab').click();
        } catch (e) {
            alert("Error al liberar fechas.");
        }
    }
}
